...
const SearchScreen = ({ navigation }) => {
  ...
  const filterMovie = (item) => {
    ...
    if (item === "movie") {
      getMoviesFromApi(
        `https://api.themoviedb.org/3/search/movie?api_key=8e612f97d036e6a7ee466a431543af09&language=en-US&query=${search}&page=1`
      );
      ...
      getMoviesFromApi(
        `https://api.themoviedb.org/3/search/multi?api_key=8e612f97d036e6a7ee466a431543af09&language=en-US&query=${search}&page=1`
      );
      setMovieSelection(item);
      setSelect(item);
      setToValue(0);
      ...
      getMoviesFromApi(
        `https://api.themoviedb.org/3/search/tv?api_key=8e612f97d036e6a7ee466a431543af09&language=en-US&page=1&query=${search}&include_adult=false`
      );
      setMovieSelection(item);
      setSelect(item);
      setToValue(0);
  ...
  const searchFilterList = (text) => {
    if (text) {
      getMoviesFromApi(
        `https://api.themoviedb.org/3/search/movie?api_key=8e612f97d036e6a7ee466a431543af09&language=en-US&query=${text}&page=1`
      );
  ...
  return (
    <View style={styles.container}>
      ...
      {checkvalidation && (
        <Text style={{ marginLeft: 20, bottom: 8, color: "red" }}>
          please search movie name{" "}
        </Text>
      )}
      <Text style={{ marginLeft: 20, bottom: 8 }}>
      ...
      <Text style={{ marginLeft: 20, bottom: 8, fontSize: 12 }}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            return (
              ...
              >
                <Text style={[styles.fltrTxt, {}]}>{item.heading}</Text>
                {/* <Text style={styles.title}>Title</Text>
            <Image
                style={styles.image}
                source={{ uri: 'https://image.tmdb.org/t/p/w154/t79ozwWnwekO0ADIzsFP1E5SkvR.jpg'}}
